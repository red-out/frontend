...
import mockData from '../mocks/cashbackData'; // Моки для данных кешбэка
...
const CashbackPage: React.FC = () => {
  ...
  const searchTerm = useSelector((state: any) => state.cashback.searchTerm);
  ...
  const fetchData = async (search: string = '') => {
    ...
    try {
      ...
      setFilteredServices(mockData); // Используем моки
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleAddToDraft = async (serviceId: number) => {
    try {
      ...
      if (error.response && error.response.status === 409) {
        alert('Категория уже добавлена в кешбэк!');
      } else {
        console.error(error);
        alert('Ошибка при добавлении в кешбэк.');
      }
    }
  };
    ...
    <div className="cashback-page">
      ...
      <div className="search-container">
        <form onSubmit={handleSearch} className="search-form">
          ...
          <Link
            to={draftOrderId ? `/monthly_cashbacks/${draftOrderId}` : '#'}
      ...
      {error && <div className="error">{error} <button onClick={() => fetchData(searchTerm)}>Повторить</button></div>}
...
//           <button type="submit" className="search-button">Найти</button>
//           <Link
//             to={draftOrderId ? `/monthly_cashbacks/${draftOrderId}` : '#'} // Изменили ссылку
//             className={`search-button link-button ${!isAuthenticated || !draftOrderId ? 'btn-disabled' : ''}`}
//             style={{ pointerEvents: isAuthenticated && draftOrderId ? 'auto' : 'none' }}
//           >
//             Ваши кешбэки{cashbacksCount ? `: ${cashbacksCount}` : ''}
